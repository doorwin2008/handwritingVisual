...
void PredicationStart(cv::Mat& frame)
{
	...
	g_ResultString = "识别结果:" + to_string(max_index) + " 可信度:" + to_string(maxVal);
...
int main(int argc, const char* argv[])
{
	...
	//加载ann模型
	 ...
	 //热力值颜色显示算法
	 //输入权重越接近于1，笔迹经过的可能性越大 
	 for (int j = 0; j < 784; j++) {
		 g_inputLayerDraw784_w[j].v =  weights[0].at<double>(0, (2 * j)) + weights[0].at<double>(0, (2 * j + 1));
		 unsigned int t = (unsigned int)g_inputLayerDraw784_w[j].v & 0x00FF;
		 unsigned int d = (unsigned int)g_inputLayerDraw784_w[j].v & 0xFF00;
		 if (t <= 10 &&  d ==0) {
			 doubleColorValue_bg_B = 255; //蓝
			 doubleColorValue_bg_G = (200-25*t) << 8;
			 doubleColorValue_bg_R = (200 -25* t) << 16;//doubleColorValue_R<< 16
		 }
		 else if (t > 10 && d == 0) { //绿
			 doubleColorValue_bg_B = t ;
			 doubleColorValue_bg_G = 180 << 8;
			 doubleColorValue_bg_R = t << 16 ;//doubleColorValue_R<< 16
		 }
		 else if ( d > 0xFF) {// 红
			 doubleColorValue_bg_B = t; 
			 doubleColorValue_bg_G = d << 8; ;
			 doubleColorValue_bg_R = 150 << 16;//doubleColorValue_R<< 16
		 }
		 else {
			 cout << "empty" << endl;
		 }
		 doubleColorValue_bg_A = 128 << 24;
		 filColor_arry[j] = doubleColorValue_bg_B| doubleColorValue_bg_G| doubleColorValue_bg_R | doubleColorValue_bg_A;
		 //cout << g_inputLayerDraw784_w[j].v << endl;
	 }
	...
	while (windowsShow) {
		...
		//cvui::text(frame, 226, 13, "ANN Handwriting Visualization",1,1);
		 ...
		 cvZH::putTextZH(frame, "隐藏层 64", cv::Point(25, 264), CV_RGB(0, 0, 0), 20);
